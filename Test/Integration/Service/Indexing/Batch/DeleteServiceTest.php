...
namespace Klevu\Configuration\Test\Integration\Service\Indexing\Batch;
...
class DeleteServiceTest extends TestCase
{
    ...
    public function testGetUserAgentProvider_ReturnsUserAgentProvider(): void
    {
        ...
        $this->assertStringMatchesFormat(
            format: '%A(%Aklevu-m2-configuration/%d.%d.%d.%d%A)%A',
            string: $userAgent,
        );
        $this->assertStringMatchesFormat(
            format: '%A(%Amagento-framework/%d.%d.%d.%d%A)%A',
            string: $userAgent,
        );
        $this->assertStringMatchesFormat(
            format: '%A(%Amagento-inventory/%d.%d.%d.%d%A)%A',
            string: $userAgent,
        );
    }
}
